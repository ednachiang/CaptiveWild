...
summary(aov(formula = RelAbund ~ Group, data=pro.muc.df))
  # p-value = 0.0287
TukeyHSD(aov(formula = RelAbund ~ Group, data=pro.muc.df))
  # Summer-Torpor = 0.0929545
  # Summer-IBA = 0.0346314
  # Summer-Spring = 0.9098571
  # Torpor-IBA = 0.9948679
  # Torpor-Spring = 0.5253304
  # IBA-Spring = 0.3680455
...
ses.mpd.muc.res <- ses.mpd(t(data.frame(otu_table(phy.muc))), pd.muc.dist, null.model = "taxa.labels", abundance.weighted = F, runs = 999)
  # Calculate MPD
  # This'll take a while to run-- be patient
#save(ses.mpd.muc.res, file = "mpd.muc.RData")
#load("mpd.muc.RData")
...
otu.muc.ann <- data.frame(lab = c("a", "b", "bc", "ac"), Group = factor(c("Summer_Captive", "Torpor", "IBA", "Spring")))
...
pd.muc.ann <- data.frame(lab = c("a", "bc", "b", "ac"), Group = factor(c("Summer_Captive", "Torpor", "IBA", "Spring")))
...
```{r}
tiff("CaptiveSeasons.muc.adiv.tiff", width = 170, height = 60, units = "mm", res = 600)
grid.newpage()
pushViewport(viewport(layout=grid.layout(2,4, widths = c(1, 1, 1, 1.4), heights = c(0.07, 1.5))))
grid.text("A", x=unit(0.03, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1),
          gp=gpar(fontsize=8,fontface="bold"))
...
# Adjust betadisper p-values
p.adjust(c(0.501,0.606,0.731,0.817,0.312,0.385), method = "fdr")
  # Summer-Torpor = 0.817
  # Summer-IBA = 0.817
  # Summer-Spring = 0.817
  # Torpor-IBA = 0.817
...
# Adjust PERMANOVA p-values
p.adjust(c(0.0014,8e-04,0.1192,0.9865,0.0162,0.0095), method = "fdr")
...
# Adjust betadisper p-values
p.adjust(c(0.11,0.066,0.088,0.721,0.38,0.614), method = "fdr")
  # Summer-Torpor = 0.220
  # Summer-IBA = 0.220
  # Summer-Spring = 0.220
  # Torpor-IBA = 0.721
...
# Adjust PERMANOVA/ANOSIM p-values
p.adjust(c(0.0027,5e-04,0.5296,0.844,0.0697,0.0163), method = "fdr")
...
### Adjust betadisper p-values
p.adjust(c(0.958,0.917,0.933,0.958,0.137,0.354), method = "fdr")
  # Summer-Torpor = 0.958
  # Summer-IBA = 0.958
  # Summer-Spring = 0.958
  # Torpor-IBA = 0.958
...
### Adjust PERMANOVA p-values
p.adjust(c(0.0016,7e-04,0.0791,0.9815,0.0178,0.0084), method = "fdr")
...
```{r}
tiff("CaptiveSeasons.muc.bdiv.tiff", width = 85, height = 180, units = "mm", res = 600)
grid.newpage()
pushViewport(viewport(layout=grid.layout(3,2, widths = c(1, 0.45), heights = c(1, 1, 1))))
...
#simper.pretty(data.frame(otu_table(phy.muc.simp)),
              ...
              c("GroupRename"),
              perc_cutoff=0.5, low_cutoff='y', low_val=0.01,
              output_name='CaptiveSeason_muc')
#group.simper <- read.csv("CaptiveSeason_muc_clean_simper.csv")
...
#group.simper.kw <- read.csv("CaptiveSeason_muc_krusk_simper.csv")
for(i in 1:nrow(group.simper.kw)){
...
#write.table(group.simper.kw, "CaptiveSeason_muc_krusk_simper.csv", sep=",")
#group.simper.kw <- read.csv("group_clean_simper.csv", row.names = 1)
```
...
#write.csv(data.frame(tax_table(phy.muc.core)), "CaptiveSeasons_muc_coreOTUs.csv")
```