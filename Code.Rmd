...
```
### Alpha Diversity - Lab squirrels separated by content/mucosa for group
```{r}
adiv.lab <- data.frame(sample_data(physeq.lab))
...
adiv.combined <- rbind(adiv.lab.con, adiv.lab.muc)

### Chao
adiv.chao <- ggplot(adiv.combined, aes(x = Group, y = chao, fill = Group)) +
...
### Shannon
adiv.shannon <- ggplot(adiv.combined, aes(x = Group, y = shannon, fill = Group)) +
  facet_grid(. ~ Sample_Type) +
  geom_dotplot(binaxis = 'y', stackdir = 'center', dotsize = 1, stackratio = 1.15) +
...
### Combine plots
tiff("adiv.tiff", width = 10, height = 9, units = "in", res = 600)
grid.newpage()
pushViewport(viewport(layout=(grid.layout(4, 1, widths = c(18), heights = c(0.75,10,1,10)))))

print(adiv.chao, vp=viewport(layout.pos.row=2, layout.pos.col=1))
print(adiv.shannon, vp=viewport(layout.pos.row=4, layout.pos.col=1))

grid.text("A", x=unit(0.015, "npc"), y=unit(-0.5, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=22,fontface="bold"))
grid.text("ns", x=unit(0.275, "npc"), y=unit(-1.89, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=15))
grid.text("ns", x=unit(0.66, "npc"), y=unit(-1.89, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=15))

grid.text("B", x=unit(0.014, "npc"), y=unit(-0.5, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1),gp=gpar(fontsize=22,fontface="bold"))
grid.text("ab            a              a             b", x=unit(0.265, "npc"), y=unit(-1.4, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1), gp=gpar(fontsize=15))
grid.text("ns", x=unit(0.66, "npc"), y=unit(-1.4, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1), gp=gpar(fontsize=15))
...
#nmds.lab.con <- ordinate(physeq=physeq.lab.con, method="NMDS", distance="bray")
  # stress = 0.1090854    
#save(nmds.lab.con, file = "nmds.lab.con.RData")
#nmds.lab.muc <- ordinate(physeq=physeq.lab.muc, method="NMDS", distance="bray")
  # stress = 0.144648
#save(nmds.lab.muc, file = "nmds.lab.muc.RData")

load("nmds.lab.con.RData")
load("nmds.lab.muc.RData")
...
# Content Ellipses
elip.nmds.lab.con <- data.frame(MDS1 = nmds.lab.con$points[,1], MDS2 = nmds.lab.con$points[,2],
                        Group = sample_data(physeq.lab.con)$Group)
plot.new()
elip.ord.lab.con <- ordiellipse(nmds.lab.con, sample_data(physeq.lab.con)$Group, display="sites", kind="se", conf=0.95, label=T)
elip.lab.con.df <- data.frame()
for(i in levels(elip.nmds.lab.con$Group)){
  elip.lab.con.df <- rbind(elip.lab.con.df, cbind(as.data.frame(with(elip.nmds.lab.con[elip.nmds.lab.con$Group==i,],
                                                     veganCovEllipse(elip.ord.lab.con[[i]]$cov,
                                                                     elip.ord.lab.con[[i]]$center,
                                                                     elip.ord.lab.con[[i]]$scale))),
                                  group=i))
}
...
plot_ordination(physeq=physeq.lab.con, ordination=nmds.lab.con, color="Group") +
  ...
  geom_point(size = 3) +
  ggtitle("Content") +
  geom_path(data=elip.lab.con.df, aes(x=NMDS1, y=NMDS2, color=group), size=1, linetype=1) +
   theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
### Mucosa Ellipse
elip.nmds.lab.muc <- data.frame(MDS1 = nmds.lab.muc$points[,1], MDS2 = nmds.lab.muc$points[,2],
                        Group = sample_data(physeq.lab.muc)$Group)
plot.new()
elip.ord.lab.muc <- ordiellipse(nmds.lab.muc, sample_data(physeq.lab.muc)$Group, display="sites", kind="se", conf=0.95, label=T)
elip.lab.muc.df <- data.frame()
for(i in levels(elip.nmds.lab.muc$Group)){
  elip.lab.muc.df <- rbind(elip.lab.muc.df, cbind(as.data.frame(with(elip.nmds.lab.muc[elip.nmds.lab.muc$Group==i,],
                                                     veganCovEllipse(elip.ord.lab.muc[[i]]$cov,
                                                                     elip.ord.lab.muc[[i]]$center,
                                                                     elip.ord.lab.muc[[i]]$scale))),
                                  group=i))
}
