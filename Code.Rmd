...
sum.goodphy <- tax_glom(physeq.sum, taxrank="Phylum")

# Transform each merged phylum count to rel abund
sum.phy.ra <- transform_sample_counts(sum.goodphy, function(x){(x/sum(x))*100})

# Create rank abundance of phyla
barplot(sort(taxa_sums(sum.phy.ra),TRUE)[1:20]/nsamples(sum.phy.ra),las=2,cex.axis=.7)

# Summarize data
sum.phy.df <- psmelt(sum.phy.ra)
