...
barplot(sort(taxa_sums(chow.phy99),TRUE)[1:20]/nsamples(phy99),las=2,cex.axis=.7)

chow.df <- psmelt(chow.phy99)
chow.df$Phylum <- ordered(chow.df$Phylum, levels=c("Proteobacteria", "Firmicutes", "Actinobacteria", "Bacteroidetes"))
...
### Chao
# Color by group
tiff("Chao.tiff", width=6.5, height=3.5, units="in", res=600)
...
dev.off()
# Color by group, facet by sample type
#tiff("Chao.tiff", width=11, height=3.5, units="in", res=600)
ggplot(adiv, aes(x=Group, y=chao, fill = Group)) +
  geom_boxplot(position=position_dodge(1)) +
  geom_point() + 
  facet_grid(.~Sample_Type) +
...
### Barplot ###
tiff("Phyla2.tiff", width=13, height=8, units="in", res=600)
