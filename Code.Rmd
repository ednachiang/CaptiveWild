...
for(i in 1:nrow(sum.simper.kw)){
  save <- which(rownames(data.frame(tax_table(physeq.sum))) == sum.simper.kw$OTU[i])
  sum.simper.kw$Taxonomy[i] <-
    paste(data.frame(tax_table(physeq.sum))[save,2], "-",
          data.frame(tax_table(physeq.sum))[save,3], "-",
          data.frame(tax_table(physeq.sum))[save,4], "-",
          data.frame(tax_table(physeq.sum))[save,5], "-",
          data.frame(tax_table(physeq.sum))[save,6])
}
...
```{r}
# Subset sig OTUs
physeq.sum.simp.ra <- transform_sample_counts(physeq.sum, function(x){(x/sum(x))*100})
