...
### Alpha Div - Adjust P
```{r}
# Stats order is same as alpha div plots
...
wild.otu.plot <- ggplot(uniq.otus.wild, aes(x = Sample_Type, y = Unique_OTUs, fill = Sample_Type)) +
  ...
  ylab("Number of Observed OTUs") + xlab("Sample Type") + labs(fill = "Sample Type") +
  ggtitle("Summer Wild") +
...
spr.mpd.plot <- ggplot(mpd.spr.ready, aes(x = Sample_Type, y = Stat, fill = Sample_Type)) +
  ...
  geom_dotplot(binaxis = 'y', stackdir = 'center', dotsize = 1, stackratio = 1.15, fill = "black") +
  scale_fill_manual(values = c("gray60", "gray90"), labels = c("Content", "Mucosa")) +
  scale_y_continuous(limits = c(-2.8,2.6), breaks = c(-2.70, -1.45, -0.10,1.25, 2.60)) +
  scale_x_discrete(labels = c("Content", "Mucosa")) +
  ylab("Phylogenetic Evenness (MPD)") + xlab("Sample Type") + labs(fill = "Sample Type") +
  ggtitle("Spring") +
    theme(panel.grid.major = element_blank(),
...
```{r}
png("FigS1.ConMuc.adiv.png", width = 170, height = 225, units = "mm", res = 600)
grid.newpage()
pushViewport(viewport(layout=grid.layout(10,4, widths = c(1, 1, 1, 1.3),
...
# Order = Weighted UniFrac ID, Weighted UniFrac Sample_Type, Unweighted UniFrac ID, Unweighted UniFrac Sample_Type, Bray-Curtis ID, Bray-Curtis Sample_Type
# PERMANOVA
...
ord.legend <- g_legend(spr.bc.plot)
```
...
### Plot - Beta-Diversity
```{r}
#png("FigS2.ConMuc.bdiv.png", width = 170, height = 225, units = "mm", res = 600)
grid.newpage()
...
### PD
cohen.d(pd.wild.ready$Stat ~ pd.wild.ready$Sample_Type, paired = T)
  # d = -0.9022288 (large)
  # 95% CI = -4.142542  2.338084 

### MPD
cohen.d(mpd.wild.ready$Stat ~ mpd.wild.ready$Sample_Type, paired = T)
  # d = 0.5213095 (medium)
  # 95% CI = 0.0497683 0.9928506 

### Shannon
cohen.d(adiv.wild$shannon ~ adiv.wild$Sample_Type, paired = T)
  # d = 0.2315921 (small)
  # 95% CI = -0.1288982  0.5920824 

### Unique OTUs
cohen.d(uniq.otus.wild$Unique_OTUs ~ uniq.otus.wild$Sample_Type, paired = T)
  # d = -0.6473075 (medium)
  # 95% CI = -2.762275  1.467660 
...
### PD
cohen.d(pd.sum.ready$Stat ~ pd.sum.ready$Sample_Type, paired = T)
  # d = 0.04735248 (negligible)
  # 95% CI = -1.006736  1.101441 

### MPD
cohen.d(mpd.sum.ready$Stat ~ mpd.sum.ready$Sample_Type, paired = T)
  # d = -0.7418809 (medium)
  # 95% CI = -2.4111945  0.9274327 

### Shannon
cohen.d(adiv.sum$shannon ~ adiv.sum$Sample_Type, paired = T)
  # d = -0.2417263 (small)
  # 95% CI = -1.612390  1.128937 

### Unique OTUs
cohen.d(uniq.otus.sum$Unique_OTUs ~ uniq.otus.sum$Sample_Type, paired = T)
  # d = 0.1704868 (negligible)
  # 95% CI = -0.9770042  1.3179778
...
### PD
cohen.d(pd.tor.ready$Stat ~ pd.tor.ready$Sample_Type, paired = T)
  # d = -0.2859916 (small)
  # 95% CI = -1.251544  0.679561

### MPD
cohen.d(mpd.tor.ready$Stat ~ mpd.tor.ready$Sample_Type, paired = T)
  # d = 0.169756 (negligible)
  # 95% CI = -0.3499992  0.6895111

### Shannon
cohen.d(adiv.tor$shannon ~ adiv.tor$Sample_Type, paired = T)
  # d = 0.1581537 (negligible)
  # 95% CI = -1.837849  2.154156

### Unique OTUs
cohen.d(uniq.otus.tor$Unique_OTUs ~ uniq.otus.tor$Sample_Type, paired = T)
  # d = 0.1861612 (negligible)
  # 95% CI = -0.6764332  1.0487557
...
### PD
cohen.d(pd.iba.ready$Stat ~ pd.iba.ready$Sample_Type, paired = T)
  # d = 0.02188684 (negligible)
  # 95% CI = -0.7789911  0.8227648

### MPD
cohen.d(mpd.iba.ready$Stat ~ mpd.iba.ready$Sample_Type, paired = T)
  # d = 0.1399599 (negligible)
  # 95% CI = -0.1528129  0.4327328 

### Shannon
cohen.d(adiv.iba$shannon ~ adiv.iba$Sample_Type, paired = T)
  # d = -0.2185459 (small)
  # 95% CI = -0.9537933  0.5167016

### Unique OTUs
cohen.d(uniq.otus.iba$Unique_OTUs ~ uniq.otus.iba$Sample_Type, paired = T)
  # d = 0.2809209 (small)
  # 95% CI = -0.8090023  1.3708440 
...
### PD
cohen.d(pd.spr.ready$Stat ~ pd.spr.ready$Sample_Type, paired = T)
  # d = -0.1633663 (negligible)
  # 95% CI = -0.7639579  0.4372254

### MPD
cohen.d(mpd.spr.ready$Stat ~ mpd.spr.ready$Sample_Type, paired = T)
  # d = -0.4335896 (small)
  # 95% CI = -1.3832070  0.5160279 

### Shannon
cohen.d(adiv.spr$shannon ~ adiv.spr$Sample_Type, paired = T)
  # d = 0.2428866 (small)
  # 95% CI = -0.3900956  0.8758689 

### Unique OTUs
cohen.d(uniq.otus.spr$Unique_OTUs ~ uniq.otus.spr$Sample_Type, paired = T)
  # d = 0.1406595 (negligible)
  # 95% CI = -0.8939879  1.1753069
...
# PD
pwr.t2n.test(length(which(adiv.wild$Sample_Type == "Content")),
             length(which(adiv.wild$Sample_Type == "Mucosa")), d = -0.9022288)
  # 0.1383985

# MPD
pwr.t2n.test(length(which(adiv.wild$Sample_Type == "Content")),
             length(which(adiv.wild$Sample_Type == "Mucosa")), d = 0.5213095)
  # 0.07920397

# Shannon
pwr.t2n.test(length(which(adiv.wild$Sample_Type == "Content")),
             length(which(adiv.wild$Sample_Type == "Mucosa")), d = 0.2315921)
  # 0.05572493

# Unique OTUs
pwr.t2n.test(length(which(adiv.wild$Sample_Type == "Content")),
             length(which(adiv.wild$Sample_Type == "Mucosa")), d = -0.6473075)
  # 0.09519332
...
# PD
pwr.t2n.test(length(which(adiv.sum$Sample_Type == "Content")),
             length(which(adiv.sum$Sample_Type == "Mucosa")), d = 0.04735248)
  # 0.05076575

# MPD
pwr.t2n.test(length(which(adiv.sum$Sample_Type == "Content")),
             length(which(adiv.sum$Sample_Type == "Mucosa")), d = -0.7418809)
  # 0.2483309

# Shannon
pwr.t2n.test(length(which(adiv.sum$Sample_Type == "Content")),
             length(which(adiv.sum$Sample_Type == "Mucosa")), d = -0.2417263)
  # 0.07016487

# Unique OTUs
pwr.t2n.test(length(which(adiv.sum$Sample_Type == "Content")),
             length(which(adiv.sum$Sample_Type == "Mucosa")), d = 0.1704868)
  # 0.0599777
...
# PD
pwr.t2n.test(length(which(adiv.tor$Sample_Type == "Content")),
             length(which(adiv.tor$Sample_Type == "Mucosa")), d = -0.2859916)
  # 0.06857708

# MPD
pwr.t2n.test(length(which(adiv.tor$Sample_Type == "Content")),
             length(which(adiv.tor$Sample_Type == "Mucosa")), d = 0.169756)
  # 0.05650604

# Shannon
pwr.t2n.test(length(which(adiv.tor$Sample_Type == "Content")),
             length(which(adiv.tor$Sample_Type == "Mucosa")), d = 0.1581537)
  # 0.05564455

# Unique OTUs
pwr.t2n.test(length(which(adiv.tor$Sample_Type == "Content")),
             length(which(adiv.tor$Sample_Type == "Mucosa")), d = 0.1861612)
  # 0.05782966
...
# PD
pwr.t2n.test(length(which(adiv.iba$Sample_Type == "Content")),
             length(which(adiv.iba$Sample_Type == "Mucosa")), d = 0.02188684)
  # 0.05016354

# MPD
pwr.t2n.test(length(which(adiv.iba$Sample_Type == "Content")),
             length(which(adiv.iba$Sample_Type == "Mucosa")), d = 0.1399599)
  # 0.05671242

# Shannon
pwr.t2n.test(length(which(adiv.iba$Sample_Type == "Content")),
             length(which(adiv.iba$Sample_Type == "Mucosa")), d = -0.2185459)
  # 0.06645192

# Unique OTUs
pwr.t2n.test(length(which(adiv.iba$Sample_Type == "Content")),
             length(which(adiv.iba$Sample_Type == "Mucosa")), d = 0.2809209)
  # 0.07732905
...
# PD
pwr.t2n.test(length(which(adiv.spr$Sample_Type == "Content")),
             length(which(adiv.spr$Sample_Type == "Mucosa")), d = -0.1633663)
  # 0.05284601

# MPD
pwr.t2n.test(length(which(adiv.spr$Sample_Type == "Content")),
             length(which(adiv.spr$Sample_Type == "Mucosa")), d = -0.4335896)
  # 0.07015416

# Shannon
pwr.t2n.test(length(which(adiv.spr$Sample_Type == "Content")),
             length(which(adiv.spr$Sample_Type == "Mucosa")), d = 0.2428866)
  # 0.0562981

# Unique OTUs
pwr.t2n.test(length(which(adiv.spr$Sample_Type == "Content")),
             length(which(adiv.spr$Sample_Type == "Mucosa")), d = 0.1406595)
  # 0.05210932
```



### Detectable Effect Size - Alpha Div
```{r}
# https://cran.r-project.org/web/packages/pwr/vignettes/pwr-vignette.html

# Summer Wild
pwr.t2n.test(length(which(adiv.wild$Sample_Type == "Content")),
             length(which(adiv.wild$Sample_Type == "Mucosa")), power = 0.8)
  # 3.070917


# Summer Captive
pwr.t2n.test(length(which(adiv.sum$Sample_Type == "Content")),
             length(which(adiv.sum$Sample_Type == "Mucosa")), power = 0.8)
  # 1.631722


# Torpor
pwr.t2n.test(length(which(adiv.tor$Sample_Type == "Content")),
             length(which(adiv.tor$Sample_Type == "Mucosa")), power = 0.8)
  # 2.024439


# IBA
pwr.t2n.test(length(which(adiv.iba$Sample_Type == "Content")),
             length(which(adiv.iba$Sample_Type == "Mucosa")), power = 0.8)
  # 1.631722


# SPRING
pwr.t2n.test(length(which(adiv.spr$Sample_Type == "Content")),
             length(which(adiv.spr$Sample_Type == "Mucosa")), power = 0.8)
  # 3.070917
