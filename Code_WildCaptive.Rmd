...
#rename <- data.frame(sample_data(phy.con))
#rename$GroupRename <- rep(NA, nrow(rename))
...
```{r}
# Change Group names to be compatible (no "_")
#rename <- data.frame(sample_data(phy.muc))
#rename$GroupRename <- rep(NA, nrow(rename))
#sum.rename$GroupRename[which(sum.rename$Group == "Summer_Wild")] <- "SummerWild"
#sum.rename$GroupRename[which(sum.rename$Group == "Summer_Lab")] <- "SummerLab"
#sum.rename$GroupRename[which(sum.rename$Group == "Torpor")] <- "Torpor"
...
group.simper <- read.csv("wild.captive.muc_clean_simper.csv")
...
group.simper.kw <- read.csv("wild.captive.muc_krusk_simper.csv")
#for(i in 1:nrow(group.simper.kw)){
  save <- which(rownames(data.frame(tax_table(phy.muc))) == group.simper.kw$OTU[i])
  group.simper.kw$Taxonomy[i] <-
...
#write.table(group.simper.kw, "wild.captive.muc_krusk_simper.csv", sep=",")
#group.simper.kw <- read.csv("wild.captive.muc_clean_simper.csv", row.names = 1)
```
