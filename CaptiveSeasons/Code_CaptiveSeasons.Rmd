...
#write.csv(data.frame(tax_table(phy.con.core)), "Content/CaptiveSeasons_con_coreOTUs.csv")
```
...
```{r}
#("Mucosa/FigS5.CaptiveSeasons.muc.adiv.png", width = 170, height = 60, units = "mm", res = 600)
grid.newpage()
pushViewport(viewport(layout=grid.layout(2,4, widths = c(1, 1, 1, 1.4), heights = c(0.07, 1.5))))
grid.text("A", x=unit(0.03, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1),
          gp=gpar(fontsize=8,fontface="bold"))
...
#write.csv(data.frame(tax_table(phy.muc.core)), "Mucosa/CaptiveSeasons_muc_coreOTUs.csv")
```