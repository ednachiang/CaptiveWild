...
# Calculate Faith's phylogenetic diversity
pd.res <- pd(t(data.frame(otu_table(physeq.pair))), phy_tree(physeq.pair), include.root = T)
...
ses.mpd.res <- ses.mpd(t(data.frame(otu_table(physeq.pair))), pd.dist, null.model = "taxa.labels", abundance.weighted = F, runs = 999)
  # Calculate MPD
...
shapiro.test(adiv.spr$chao)
  # p-value = 0.7986
  # Normal
p.adjust(c(0.001037, 0.1201, 0.03314, 0.05629, 0.003596, 0.7986), method = "fdr")
  # 0.006222* 0.144120 0.066280 0.084435 0.010788* 0.798600
  # All normal except for total & IBA

### Wilcoxon & T-Test ###
wilcox.test(chao ~ Sample_Type, data = adiv, paired = T)
  # p-value = 0.003878
t.test(chao ~ Sample_Type, data = adiv.wild, paired = T)
  # p-value = 0.2454
t.test(chao ~ Sample_Type, data = adiv.sum, paired = T)
  # p-value = 0.3271
t.test(chao ~ Sample_Type, data = adiv.tor, paired = T)
  # p-value = 0.07519
wilcox.test(chao ~ Sample_Type, data = adiv.iba, paired = T)
  # p-value = 0.1953
t.test(chao ~ Sample_Type, data = adiv.spr, paired = T)
  # p-value = 0.8478
...
shapiro.test(adiv.spr$shannon)
  # p-value = 0.09141
  # Normal
p.adjust(c(0.4286, 0.4286, 0.6448, 0.3544, 0.5667, 0.09141), method = "fdr")
  # 0.64290 0.64290 0.64480 0.64290 0.64480 0.54846
...
t.test(shannon ~ Sample_Type, data = adiv.spr, paired = T)
  # p-value = 0.6029
p.adjust(c(0.3436, 0.9706, 0.6903, 0.03376, 0.03364, 0.6029), method = "fdr")
  # 0.68720 0.97060 0.82836 0.10128 0.10128 0.82836
...
shapiro.test(adiv.spr$invsimpson)
  # p-value = 0.582
  # Normal
p.adjust(c(1.215e-05, 0.828, 0.2743, 0.06794, 0.01567, 0.582), method = "fdr")
  # 0.0000729 0.8280000 0.4114500 0.1358800 0.0470100 0.6984000
  # All normal except all and IBA

### Wilcoxon & T-Test ###
wilcox.test(invsimpson ~ Sample_Type, data = adiv, paired = T)
  # p-value = 0.2109
t.test(invsimpson ~ Sample_Type, data = adiv.wild, paired = T)
  # p-value = 0.8733
t.test(invsimpson ~ Sample_Type, data = adiv.sum, paired = T)
  # p-value = 0.5039
t.test(invsimpson ~ Sample_Type, data = adiv.tor, paired = T)
  # p-value = 0.03779*
wilcox.test(invsimpson ~ Sample_Type, data = adiv.iba, paired = T)
  # p-value = 0.03906*
t.test(invsimpson ~ Sample_Type, data = adiv.spr, paired = T)
  # p-value = 0.6307
p.adjust(c(0.2109, 0.8733, 0.5039, 0.03779, 0.03906, 0.6307), method = "fdr")
  # 0.42180 0.87330 0.75585 0.11718 0.11718 0.75684
...
plot_ordination(physeq = Ver.lau, ordination = Ver.lau.pcoa, axes = c(1, 2),
                title= "Unweighted UniFrac") + 
  geom_point(size=2, aes(color = Season, shape = Depth,
                         fill = factor(ifelse(Fraction == "Free", NA, Season)),
                         alpha = Fraction)) +
  scale_shape_manual(values = c(21, 24, 22)) +
   scale_colour_manual(values=c("#ace600","#41ab5d","#00441b")) +
  scale_fill_manual(values=c("#ace600","#41ab5d","#00441b")
