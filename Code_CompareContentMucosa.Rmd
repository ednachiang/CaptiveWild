...
pd.plot <- ggplot(pd.ready, aes(x = Sample_Type, y = Stat, fill = Sample_Type)) +
  ...
  scale_y_continuous(limits = c(13,39), breaks = c(13,18,23,28,33,38))  +
  ...
  annotate("text", x = 1.5, y = 39, label = "ns", size = 2)
...
mpd.plot <- ggplot(mpd.ready, aes(x = Sample_Type, y = Stat, fill = Sample_Type)) +
  ...
  geom_point(data = mpd.sum, aes(y= Mean), colour="white", size = 0.5, 
             show.legend = F) +
  scale_fill_manual(values = c("chocolate4", "darkgoldenrod2")) +
  scale_y_continuous(limits = c(-7,5.5), breaks = c(-7, -4.5, -2, 0.5, 3, 5.5))  +
  ylab("Phylogenetic Evenness (MPD)") + labs(fill = "Sample Type") +
    theme(panel.grid.major = element_blank(),
        ...
        legend.position = "none",
        plot.margin = margin(t=1, r=1, b=1.5, l=1, unit = "mm")) +
  annotate("text", x = 1.5, y = 5.5, label = "ns", size = 2)
...
shannon.plot <- ggplot(adiv, aes(x = Sample_Type, y = shannon, fill = Sample_Type)) +
  ...
  geom_point(data = shannon.sum, aes(y= Mean), colour="white", size = 0.5, 
             show.legend = F) +
  scale_fill_manual(values = c("chocolate4", "darkgoldenrod2")) +
  scale_y_continuous(limits = c(1.4,5.9), breaks = c(1.4,2.3,3.2,4.1,5.0, 5.9))  +
  ...
  annotate("text", x = 1.5, y = 5.8, label = "ns", size = 2)
...
u.plot <- plot_ordination(physeq = physeq.pair, ordination = UF.u.pcoa, axes = c(1,2),
                title = "Unweighted UniFrac", color = "Sample_Type", shape = "Group") +
  geom_point(size = 2) +
  #stat_ellipse(aes(colour = Sample_Type), size = 1, level = 0.95) +
  scale_color_manual(values = c("chocolate4", "darkgoldenrod2"), labels = c("Content", "Mucosa")) +
...
w.plot <- plot_ordination(physeq = physeq.pair, ordination = UF.w.pcoa, axes = c(1,2),
...
print(w.plot, vp=viewport(layout.pos.row=4, layout.pos.col=1:2))
grid.text("E", x=unit(0.1, "npc"), y=unit(-1, "npc"), 
          vp=viewport(layout.pos.row=3, layout.pos.col=1), gp=gpar(fontsize=10,fontface="bold"))
print(u.plot, vp=viewport(layout.pos.row=4, layout.pos.col=3:4))
